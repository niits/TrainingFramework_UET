...
LRESULT WINAPI ESWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		if (esContext && esContext->mouseFunc)
			esContext->mouseFunc(esContext, 0, (int)point.x, (int)point.y);
	}
	break;
	...
	case WM_LBUTTONUP:
	{
		...
		if (esContext && esContext->mouseFunc)
			esContext->mouseFunc(esContext, 1, (int)point.x, (int)point.y);
	}
	break; 
	...
	case WM_MOUSEMOVE:
		if (wParam & MK_LBUTTON)
		{
			...
			point = MAKEPOINTS(lParam);
			if (esContext && esContext->mouseFunc)
				esContext->mouseFunc(esContext, 2, (int)point.x, (int)point.y);
		}
		break;	
